...
class TrainDataset_Inpainting(Dataset):
    ...
    def __init__(self, dataroot, seqlen, dropout):
        ...
        self.indice = []
        # load file names
        self.fnamesTrainX, self.fnamesTrainCond = self._make_dataset(dataroot)
        #
        self.EEindex_u = [36,37,38,
                    48,49,50,
                    60,61,62 ]
        self.EEindex_l = [24,25,26,
                    12,13,14]
    ...
    def _make_dataset(self, root):
        ...
        return fnamesTrainX, fnamesTrainCond
    ...
    def __getitem__(self, idx):
        ...
        if self.dropout>0.:
            ...
            #ee_cond_all_upper = np.repeat(self.zeromask_ee[:,0:0+1],tt,axis=-1)
            #ee_cond_all_upper = np.tile(self.zeromask_ee[:,0:0+1],(1,tt))
            ...
            sample = {'x': self.x[:,:], 'cond': cond_masked, 'ee_cond' :ee_cond_masked}
        else:
            sample = {'x': self.x[:,:], 'cond': self.cond[:,:],'ee_cond':self.ee_cond}
...
class ValidDataset_Inpainting(Dataset):
    ...
    def __init__(self, dataroot, seqlen, dropout):
        ...
        # load file names
        self.fnamesValidX, self.fnamesValidCond = self._make_dataset(dataroot)
    ...
    def _make_dataset(self, root):
        ...
        return fnamesValidX, fnamesValidCond                                                                     
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        if self.dropout>0.:
            ...
            # end effector
            ee_cond_masked = self.ee_cond[:,:].copy()
            mask_ee_cond = np.full((15, tt), False)
            p = np.random.rand(1)
            if p<(0.2):
                mask_ee_cond[:3,:] = True
            if p>(0.2) and p <0.4:
                mask_ee_cond[3:6,:] = True
            if p>(0.4) and p <0.6:
                mask_ee_cond[6:9,:] = True
            if p>(0.6) and p <0.8:
                mask_ee_cond[9:12,:] = True
            if p>(0.8):
                mask_ee_cond[12:15,:] = True
...
class TestDataset_Inpainting(Dataset):
    ...
    def __init__(self, dataroot, seqlen,dropout):
        ...
        # load path
        self.fnamesTest = self._make_dataset(dataroot)
    
    def _make_dataset(self, root):
        fnamesTest = []
        for filename in sorted(glob.glob(root + '/test_scaled_*.npz')):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'autoreg': self.autoreg[:,:], 'cond': self.cond}
